...
export const agentTools = {
  get_schema: tool({
    ...
    execute: async () => {
      return {
        tables: {
          activities: {
            description: "Mirror of Strava activities",
            columns: {
          ...
          conversations: {
            ...
            columns: {
              id: "uuid",
              title: "text nullable — auto-generated after first response",
              created_at: "timestamptz",
            },
          },
          messages: {
            ...
            columns: {
              ...
              content: "jsonb — structured for assistant messages, plain text for user",
              created_at: "timestamptz",
            },
          },
          agent_traces: {
            description: "Full trace of each agent turn for future evals",
            columns: {
              id: "uuid",
  ...
  run_query: tool({
    description:
      "Executes a read-only SQL query against the activities database. Use for all data retrieval. SQL must be SELECT only — no mutations.",
    inputSchema: z.object({
      sql: z
        .string()
  ...
  get_personal_records: tool({
    ...
    execute: async () => {
      const { data, error } = await supabaseAdmin
        ...
        .select(
          "metric, value, achieved_at, activities(name, type, start_date)"
        )
        .order("metric");
      if (error) return { error: error.message };
      return data;
  ...
  add_note: tool({
    ...
    execute: async ({ content, activity_id, note_date }: { content: string; activity_id?: number; note_date?: string }) => {
      const { data, error } = await supabaseAdmin
        .from("activity_notes")
        .insert({ content, activity_id: activity_id ?? null, note_date: note_date ?? null })
        .select()
        .single();
      if (error) return { error: error.message };
