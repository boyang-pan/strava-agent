    ...
    <div className="border border-zinc-100 rounded-md overflow-hidden">
      ...
      <button
        ...
        className={cn(
          ...
          canExpand ? "hover:bg-zinc-50 cursor-pointer" : "cursor-default"
        )}
      >
        {/* Status icon */}
        <span className="shrink-0 w-4 h-4 flex items-center justify-center">
          ...
          {state.status === "pending" && (
            <Dot className="w-3.5 h-3.5 text-zinc-300" />
          )}
        </span>
        ...
        {canExpand && (
          <ChevronDown
            className={cn(
              "w-3.5 h-3.5 text-zinc-300 shrink-0 transition-transform",
              isOpen && "rotate-180"
            )}
          />
        )}
      ...
      {isOpen && state.toolCall && (
        <div className="border-t border-zinc-100 bg-zinc-50 p-3 space-y-2">
          <p className="text-xs font-medium text-zinc-500 font-tool">
            {state.toolCall.tool}
          </p>

          {state.toolCall.input !== undefined && (
            <div>
              <p className="text-xs text-zinc-400 font-tool mb-0.5">input</p>
          ...
          {state.toolCall.output !== undefined && (
            <div>
              <p className="text-xs text-zinc-400 font-tool mb-0.5">output</p>
